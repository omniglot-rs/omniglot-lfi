...
pub fn with_lfi_sysv_amd64_rt_lib<ID: OGID, R>(
    brand: ID,
    f: impl for<'a> FnOnce(
        LibPngRt<ID, omniglot_lfi::amd64::OGLFISysVAMD64Runtime<ID>, omniglot_lfi::amd64::OGLFISysVAMD64Runtime<ID>>,
        AllocScope<
    ...
    let (rt, alloc, access) = omniglot_lfi::amd64::OGLFISysVAMD64Runtime::from_lfi_lib_bytes(
        ...
        c"libpng".into(),
        [].into_iter(),
        // when used with `_default`, this causes the benchmark to
        // error when trying upgrade the target buffer reference in
        // libpng's read_fn callback
    ...
    lib.rt()
        .setup_callback(
            ...
            |callback_ptr, alloc| {
                ...
                // the `omniglot` crate is not enabled:
                #[cfg(true)]
                let row_pointers_slice =
                    OGMutSlice::upgrade_from_ptr(row_pointers_arr, row_count, alloc)
                        .unwrap_or_else(|| {
