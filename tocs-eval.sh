...
echo "========== Omniglot-LFI TOCS Evaluation Script =========="
...
| Omniglot     | ???????????????? | ${BENCH_RESULTS["runtime_setup/og_lfi"]} | ???????????????? | ${BENCH_RESULTS["ubench_invoke/og_lfi/0"]} |
| Tock Upcall  | ---------------- | ---------------- | ???????????????? | ---------------- |
EOF
...
|--------------+------------------+------------------+------------------+------------------|
|          PMP | ???????????????? | ???????????????? | ???????????????? | ???????????????? |
|          LFI | ${BENCH_RESULTS["validation/u8/64"]} | ${BENCH_RESULTS["validation/u8/8224"]} | ${BENCH_RESULTS["validation/str/64"]} | ${BENCH_RESULTS["validation/str/8224"]} |
EOF
...
BROTLI_ISOLATION="${BENCH_RESULTS["brotli-unchecked/brotli_compress_decompress/omniglot-lfi/1024"]}"
BROTLI_OMNIGLOT="${BENCH_RESULTS["brotli-checked/brotli_compress_decompress/omniglot-lfi/1024"]}"
BROTLI_ISOLATION_OVERHEAD="$(\
	compute_overhead \
		"${BENCH_RESULTS_EST["brotli-unchecked/brotli_compress_decompress/omniglot-lfi/1024"]}" \
		"${BENCH_RESULTS_EST["brotli-unchecked/brotli_compress_decompress/unsafe/1024"]}")"
BROTLI_OMNIGLOT_OVERHEAD="$(\
	compute_overhead \
		"${BENCH_RESULTS_EST["brotli-checked/brotli_compress_decompress/omniglot-lfi/1024"]}" \
		"${BENCH_RESULTS_EST["brotli-unchecked/brotli_compress_decompress/omniglot-lfi/1024"]}")"

SODIUM_UNSAFE="${BENCH_RESULTS["sodium-unchecked/libsodium_hash/unsafe/32768"]}"
SODIUM_ISOLATION="${BENCH_RESULTS["sodium-unchecked/libsodium_hash/og_lfi/32768"]}"
SODIUM_OMNIGLOT="${BENCH_RESULTS["sodium-checked/libsodium_hash/og_lfi/32768"]}"
SODIUM_ISOLATION_OVERHEAD="$(\
	compute_overhead \
		"${BENCH_RESULTS_EST["sodium-unchecked/libsodium_hash/og_lfi/32768"]}" \
		"${BENCH_RESULTS_EST["sodium-unchecked/libsodium_hash/unsafe/32768"]}")"
SODIUM_OMNIGLOT_OVERHEAD="$(\
	compute_overhead \
		"${BENCH_RESULTS_EST["sodium-checked/libsodium_hash/og_lfi/32768"]}" \
		"${BENCH_RESULTS_EST["sodium-unchecked/libsodium_hash/og_lfi/32768"]}")"

LIBPNG_UNSAFE="${BENCH_RESULTS["libpng-unchecked/libpng_decode/unsafe/big_building_001.0p.png"]}"
LIBPNG_ISOLATION="${BENCH_RESULTS["libpng-unchecked/libpng_decode/og_lfi/big_building_001.0p.png"]}"
LIBPNG_OMNIGLOT="${BENCH_RESULTS["libpng-checked/libpng_decode/og_lfi/big_building_001.0p.png"]}"
